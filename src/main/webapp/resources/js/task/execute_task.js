$(document).ready(function() {
	...
	$('.a_view').click(function() {
		...
		$.ajax({
			...
			success: function(data) {
				if(data) {
					...
					$('#feedback').val(data.executeFeedback);
					$('#op_id').val(data.id);
					...
					if(data.executeFileName) {
						$('.executefilename').html(data.executeFileName);
					}
				}
			}
		});
		...
		if($('.editBox').attr('display') != 'none') {
			window.content_editor = KindEditor.create('textarea[name="executeFeedback"]', {
			   resizeType: 1,
			   allowPreviewEmoticons: false,
			   allowImageUpload: false,
			   afterCreate: function() {
				   this.sync();
				   this.html($('#feedback').val());
			   },
		...
		$.ajaxFileUpload({
			...
			success: function(data, status) {
				if(status == 'error') {
					...
					$('.executefilename').html(data.file_name);
					$('#executeFileName').val(data.file_name);
					$('#executeFileURL').val(data.file_url);
				}
			}
		});
	});
